    ...
    if request.method == "PUT":
        with sqlite3.connect('blog.db') as conn:
            ...
            if incoming_data.get('title') is not None:
                ...
                with sqlite3.connect("blog.db") as conn:
                    cursor = conn.cursor()
                    cursor.execute("UPDATE post SET title =? WHERE id =?", (put_data["title"], post_id["title"]))
                    conn.commit()
                ...
                if incoming_data.get("content") is not None:
                    ...
                    with sqlite3.connect("blog.db") as conn:
                        cursor = conn.cursor()
                        cursor.execute("UPDATE post SET content =? WHERE id =?",
                                       (put_data['content'], post_id['content']))
                        conn.commit()
